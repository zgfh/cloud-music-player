...
async def _fill_credentials(tester, url, username, password):
    ...
    await tester.enter_text(await tester.find_by_key("nextcloud_sync_folder"), "/music")
...
async def test_nextcloud_sync_download_and_playback(
    ...
    await _fill_credentials(tester, mock_nextcloud_server.url, USERNAME, PASSWORD)
...
async def test_nextcloud_wrong_password_shows_error_then_recovers(
    ...
    error = await wait_for(tester, lambda: tester.find_by_text_containing("连接失败"))
